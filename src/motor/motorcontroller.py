    ...
    def rotate(self, angle, verbose=False):
        ...
        with self.lock:
            if self.motor_thread is not None:
                ...
                if verbose:
                    print("angle_traversed", angle_traversed)
            ...
            self.motor_thread = threading.Thread(target=self.motor.motor_run, args=(self.pins,), kwargs={'steps': abs(steps), 'ccwise':steps<0})
